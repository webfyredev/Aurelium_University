    ...
    return(
        <>
            ...
            <div className="w-full flex flex-col items-center p-5">
                <div className="w-full lg:w-[90%] lg:p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    {research_card.map((data, index) => (
                        <motion.div 
                            {...scrollRightEffects}
                            key={index} className="p-4 lg:p-5 flex flex-col items-center rounded-xl shadow-sm bg-purple-50 hover:bg-purple-100 cursor-pointer transition-all duration-300">
            ...
            <div className="p-5 lg:p-10 w-full flex flex-col items-center">
                ...
                <div className="w-full lg:w-[90%] mt-5 lg:p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-5">
                    {research_facilities.map((facitlities, index) => (
                        <motion.div 
                            {...scrollUpEffect}
                            key={index} className="p-3 rounded-md shadow-md flex space-x-2 cursor-pointer mb-3">
            ...
            <div className="w-full p-5 lg:p-10 flex flex-col items-center bg-gradient-to-r from-slate-50 to-purple-50" id="join_labs">
                ...
                <motion.h2 {...scrollUpEffect} className="text-xl md:text-3xl font-bold text-purple-950 text-center">
                ...
                <div className="w-full lg:w-[90%] lg:p-5 mt-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-5">
                    {research_impact.map((data, index) => (
                        <motion.div 
                            {...scrollRightEffects}
                            key={index} className="rounded-md bg-white flex flex-col shadow-sm cursor-pointer overflow-hidden group">
            ...
            <div className="w-full p-5 lg:p-10 border-1 flex flex-col items-center bg-purple-950">
                <motion.h4 {...borderFadeShow} className="font-semibold text-xs text-spacing-5 mt-5 text-purple-300 mb-3">
                    COLLABORATIONS OPPORTUNITIES
                </motion.h4>
                <motion.h2 {...scrollUpEffect} className="text-2xl md:text-3xl font-bold text-white">
                    Partner With Us
                </motion.h2>
                <motion.div {...borderFadeShow} className="w-20 my-3 border-2 border-white"></motion.div>
                <motion.p {...scrollupDelayEffects} className="text-xs md:text-[13px] w-85 md:w-150 text-center mt-2 text-purple-100">
                ...
                <div className="w-full lg:w-[85%] lg:p-5 mt-5 border-white grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {research_partners.map((data, index) => (
                        <motion.div {...borderFadeShow} key={index} className="p-8 rounded-xl bg-white flex flex-col items-center">
                ...
                <motion.button 
                    {...buttonHoverEffects}
                    className="flex items-center border-1 mt-7 px-6 py-3 text-[13px] rounded-xl cursor-pointer bg-white font-semibold text-purple-950">
            ...
            <div className="w-full p-5 lg:p-10 flex flex-col items-center">
                ...
                <motion.h2 
                    {...scrollUpEffect}
                    className="text-2xl md:text-3xl font-bold text-purple-950 text-center">
                ...
                <div className="w-full lg:p-5 mt-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {research_stories.map((stories, index) =>(
                        <motion.div 
                            {...scrollRightEffects}
                            key={index} className="rounded-md bg-white shadow-sm mb-3 hover:shadow-lg transition-transform duration-300 group overflow-hidden">
                            ...
                            <div className="flex flex-col p-3 rounded-b-md">
                                ...
                                <motion.button {...buttonHoverEffects} className="border-1 mt-4 mb-2 py-2.5 text-sm rounded-md font-semibold cursor-pointer bg-gradient-to-r from-purple-500 to-purple-600 text-white">
                                    Read More
                                </motion.button>
                            </div>
            ...
            <div className="w-full p-5 lg:p-10 flex flex-col items-center bg-gradient-to-r from-slate-50 to-purple-50">
                ...
                <div className="w-full lg:w-[90%] lg:p-5 mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    {research_funding.map((funds) => (
                        <motion.div {...scrollUpEffect} className="p-5 rounded-xl shadow-sm flex flex-col items-center bg-white hover:shadow-xl transition-all duration-500">
