    ...
    return(
        <>
            ...
            <div className="w-full flex flex-col  items-center p-5">
                <div className="w-[90%] p-5 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-5">
                    {research_card.map((data, index) => (
                        <div key={index} className="p-5 flex flex-col items-center rounded-xl shadow-sm bg-purple-50 cursor-pointer">
                ...
                <div className="w-[90%] mt-5 p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-5">
                    {research_facilities.map((facitlities, index) => (
                        <div key={index} className="p-3 rounded-md shadow-md flex space-x-2 cursor-pointer mb-3">
                            <facitlities.icon  className="w-12 h-12 border-1 p-3 rounded-md bg-gradient-to-r from-purple-500 to-purple-600 text-white"/>
                        ...
                        <div key={index} className="rounded-md bg-white flex flex-col shadow-sm cursor-pointer overflow-hidden group">
                            ...
                            <div className="flex p-3 space-x-3 rounded-b-md">
                                ...
                                <div className="flex flex-col">
                                    <h3 className="font-semibold text-purple-950">{data.title}</h3>
                                    <p className="text-xs mt-0.5">{data.text}</p>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <div className="w-full p-10 border-1 flex flex-col items-center bg-purple-950">
                ...
                <p className="text-[13px] w-150 text-center mt-2 text-purple-100">
                    Join forces with our world-class researchers to drive innovation and create meaningful impact
                </p>
                <div className="w-[85%] p-5 mt-5 border-white grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {research_partners.map((data, index) => (
                        <div key={index} className="p-5 rounded-xl bg-white flex flex-col items-center">
            ...
            <div className="w-full p-10 flex flex-col items-center bg-gradient-to-r from-slate-50 to-purple-50">
                ...
                <div className="w-[90%] p-5 mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
