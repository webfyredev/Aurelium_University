...
export default function Home() {
  ...
  return (
    ...
    <motion.img {...scrollRightEffects}  src="/about/about1.webp" alt="About_Us" className="w-full lg:w-1/2 lg:h-95 md:h-100 object-cover rounded-md hover:scale-102 transition duration-300" />
    <div className="flex flex-col lg:p-5 w-full lg:w-1/2">
    <motion.h2 {...scrollUpEffect} className="font-bold text-purple-950 text-xl md:text-3xl mt-5 lg:mt-0">Welcome To Aurelium University</motion.h2>
    <motion.p {...scrollupDelayEffects} className="font-semibold mt-5 text-xl bg-gradient-to-r from-purple-500 to-purple-600 bg-clip-text text-transparent">
      Our Vission
    </motion.p>
    <motion.p {...textFadeIn} className="text-sm my-1">
    ...
    <motion.p 
      {...scrollUpEffect}
      className="font-semibold mt-5 text-xl bg-gradient-to-r from-purple-500 to-purple-600 bg-clip-text text-transparent">
      Our Mission
    </motion.p>
      ...
      <div className="w-full mt-5 lg:p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {programs.map((data, index) => (
          <motion.div key={index} 
              {...scrollRightEffects}
            className="w-full rounded-md bg-white flex flex-col shadow-sm group overflow-hidden hover:shadow-lg transition duration-300">
            ...
            <div className="flex flex-col p-3 w-full">
              <h3 className="mt-2 font-semibold text-purple-950 group-hover:text-purple-500 transition-all duration-300">
                {data.title}
              </h3>
              ...
              <ul className="space-x-2 mt-2 w-full grid sm:grid-cols-2 md:grid-cols-2 p-2 gap-2">
                  {data.bullets.map((buts, i) => (
                    <li key={i} className="text-[11px] p-2 flex items-center justify-center font-semibold text-center rounded-md bg-purple-100 text-purple-950 cursor-pointer hover:bg-gradient-to-r from-purple-500 to-purple-600 hover:text-white transition-all duration-300">{buts}</li>
                  ))}
              </ul>
              <motion.button 
                  {...buttonHoverEffects}
    ...
    <div className="flex flex-col items-center p-5 lg:p-10">
      ...
      <motion.p {...scrollupDelayEffects} className="w-85 md:w-150 text-xs md:text-sm text-center text-gray-600 mt-2">
        We provide exceptional educational experience that combines academic excellence, innovative research, and comprehensive student support to help you achieve your goals
      </motion.p>
      <div className="w-full mt-5 md:p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {success_mission.map((data) => (
          <motion.div {...scrollLeftEffects} key={data.id} className="space-y-3 p-5 rounded-sm hover:rounded-xl flex flex-col items-center shadow-sm hover:shadow-lg cursor-pointer mb-3 group transition-all duration-300">
              <data.icon key={data.id}  className="border-1 w-13 h-13 p-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg"/>
              <h3 className="text-base font-semibold text-gray-800 group-hover:text-purple-600 transition-all duration-300">
                {data.title}
              </h3>
    ...
    <div className="w-full p-10 bg-gradient-to-r from-purple-500 to-purple-600 flex flex-col items-center"> 
        <motion.h3 {...scrollUpEffect} className="text-2xl md:text-3xl font-bold mt-5 text-white">
          Excellence in Numbers
        </motion.h3>
        <motion.p {...scrollupDelayEffects} className="w-85 md:w-150 text-xs md:text-sm text-center mt-2 text-purple-100">
        ...
        <div className="w-full mt-5 p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {excellence_numbers.map((data) =>(
            <motion.div {...scrollUpEffect} key={data.id} className="flex flex-col items-center p-5 space-y-2">
    ...
    <div className="bg-white w-full p-5 lg:p-10 flex flex-col items-center">
        ...
        <div className="w-full lg:w-[90%] mt-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-5">
          {campus_life.map((campus) => (
            <motion.div 
              {...scrollRightEffects}
              key={campus.id} className="rounded-md relative flex flex-col items-center group overflow-hidden">
        ...
        <div className="w-full mt-10 lg:px-5 py-10 flex flex-col items-center" id="school_facilites">
            ...
            <motion.h2 {...scrollUpEffect} className="text-xl md:text-3xl lg:text-4xl font-bold mt-2 text-purple-900 text-center">
            ...
            <div className="w-full lg:p-5 mt-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {facilities.map((facs, index) => (
                <motion.div
                  {...scrollLeftEffects} 
                  key={index} className="rounded-md hover:rounded-xl shadow-sm hover:shadow-lg flex flex-col mb-5 cursor-pointer group overflow-hidden transition-all duration-300">
                  ...
                  <div className="flex flex-col p-3">
                    <h3 className="text-base font-semibold text-purple-950 group-hover:text-purple-500 transition-all duration-300">
                      {facs.title}
                    </h3>
