    ...
    return(
        <>
            ...
            <div className="w-full p-5 lg:p-10 flex flex-col items-center bg-gradient-to-r from-slate-50 to-purple-50" id="programsdetails">
                ...
                <motion.h2 {...scrollUpEffect} className="text-2xl md:text-3xl font-bold text-purple-950">
                    What You Need
                </motion.h2>
                <motion.div {...borderFadeShow} className="w-20 my-3 border-2 border-purple-700"></motion.div>
                <div className="w-full lg:w-[90%] lg:p-5 mt-5 grid grid-cols-1 sm:grid-cls-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {requirements.map((data) => (
                        <motion.div {...scrollRightEffects} key={data.id} className="px-5 py-8 lg:py-5 flex flex-col items-center rounded-lg shadow-md bg-white hover:shadow-lg cursor-pointer group">
                            <data.icon  className="w-13 h-13 p-3 border-1 rounded-md bg-gradient-to-r from-purple-500 to-purple-600 text-white"/>
                            <h2 className="mt-3 font-semibold text-md text-purple-950 group-hover:text-purple-500 transition-all duration-300">
                                {data.title}
                            </h2>
            ...
            <div className="w-full p-5 bg-white flex flex-col items-center">
                ...
                <motion.div {...borderFadeShow} className="w-20 my-3 border-2 border-purple-700"></motion.div>
                <motion.p {...scrollupDelayEffects} className="text-[13px] w-150 text-center mt-2 text-gray-500">
                    Complete the form below to begin your application
                </motion.p>
                <form action="" className="w-full lg:w-[70%] my-5 p-5 shadow-lg rounded-ms">
                    <div className="md:flex md:flex-row flex flex-col space-x-5 w-full mb-3">
                    ...
                    <div className="w-full mb-3 flex space-x-2">
                        <input type="checkbox" name="" id="" /><p className="md:text-xs text-[10px]">I agree to the <a href="" className="text-purple-500 font-semibold">Terms and conditions</a> and confirm that all information provided is accurate</p>
                    </div>
                    <motion.button 
                        {...buttonHoverEffects}
                        className="flex items-center justify-center border-1 w-full h-11 text-sm my-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white font-semibold rounded-md cursor-pointer">
                        Submit Application <FaPaperPlane  className="w-3 h-3 mt-0.5 ml-1"/>
                    </motion.button>
                </form>
            </div>
            <div className="w-full p-5 lg:p-10 md:flex md:flex-row flex flex-col items-center bg-gradient-to-r from-slate-50 to-purple-50 space-x-5">
                <div className="w-full md:w-1/2 p-5">
                    <motion.p {...borderFadeShow} className="w-40 text-[10px] text-green-800 py-1 text-center bg-green-200 font-semibold rounded-lg">Financial Support Available</motion.p>
                    <motion.h2 
                        {...scrollUpEffect}
                        className="text-2xl md:text-3xl lg:text-4xl font-bold mt-3 text-purple-950">
                        Scholarships & Financial Aid
                    </motion.h2>
                    <motion.p {...scrollUpEffect} className="text-xs mt-4 text-gray-600">
                    ...
                    <motion.ul {...scrollupDelayEffects}className="flex flex-col space-y-4 mt-4">
                    {aid.map((aid, index) => (
                    ...
                    <motion.button {...buttonHoverEffects} className="flex items-center border-2 border-purple-950 mt-8 px-8 py-3 text-xs text-purple-950 font-semibold rounded-md cursor-pointer hover:bg-gradient-to-r from-purple-500 to-purple-600 hover:text-white hover:border-none transition-all duration-300">Learn About More Scholarships <FaArrowRight className="ml-1 mt-0.5" /></motion.button>
                </div>
                <div className="w-full md:w-1/2 lg:p-5 flex flex-col space-y-3">
                    {scholarsips.map((data, index) => (
                        <motion.div {...scrollUpEffect} key={index} className="p-3 rounded-md shadow-sm flex flex-col space-y-2 bg-white">
            ...
            <Faqs />
            <motion.div {...scrollUpEffect} className="w-full py-5 px-5 lg:px-15 bg-purple-950 md:flex md:flex-row flex flex-col justify-between items-center">
                <motion.div {...scrollUpEffect} className="flex space-x-5 p-5 items-center">
                    <FaUsers  className="w-10 h-10 text-purple-200"/>
                    <div className="">
                        <h3 className="font-bold text-sm md:text-base lg:text-xl text-white">
                            Need Help with Your Application?
                        </h3>
                        <p className="text-[13px] text-purple-100">Our admissions team is here to assist you</p>
                    </div>
                </motion.div>
                <motion.div {...scrollUpEffect} className="flex space-x-5 p-3">
                    <motion.button {...buttonHoverEffects} className="flex items-center bg-white px-7 py-2 cursor-pointer text-[13px] text-purple-950 font-semibold rounded-md"><FaPhoneAlt  className="w-3 h-3 mt-0.5 mr-1"/> Call Us</motion.button>
                    <motion.button {...buttonHoverEffects} className="flex items-center bg-purple-500 px-7 py-2 cursor-pointer text-[13px] text-white font-semibold rounded-md"><FaEnvelope  className="w-3 h-3 mt-0.5 mr-1"/> Email Admissions</motion.button>
                </motion.div>
            </motion.div>
        </>
    );
